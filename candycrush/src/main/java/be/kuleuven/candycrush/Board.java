...
public class Board<T> {
    ...
    private Candy candy;
    public Board(Iterable<T> iter){//iterable of your (Candy) object types that represents each cell's content
    ...
    public void fill(Function<Position, T>cellCreator){//om het hele bord te vullen. De fill-functie heeft als parameter een Function-object (cellCreator) die, gegeven een Positie-object, een nieuw cel-object teruggeeft.
        ...
        typeList.set(position.toIndex(), cellContent);
    }
    public Iterable<T> copyTo(Boardsize otherBoard){//die alle cellen van het huidige bord kopieert naar het meegegeven bord. Als het meegegeven bord niet dezelfde afmetingen heeft, gooi je een exception.
        //gooi exception als het niet lukt
        if(Objects.equals(boardsize.columns(), otherBoard.columns()) && Objects.equals(boardsize.rows(), otherBoard.rows())){
